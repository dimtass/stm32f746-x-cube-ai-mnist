...
/* Uncomment to overclock to 288 MHz */
// #define OVERCLOCK
...
void dbg_uart_parser(uint8_t *buffer, size_t bufferlen);
void fb_uart_parser(uint8_t *buffer, size_t bufferlen);
/* USER CODE END PFP */
...
void main_loop()
{
    if (glb_tmr_1ms) {
        ...
        if (dbg_uart.rx_ready_tmr == 1) {
            ...
            TRACE(("Received: %s\n", dbg_uart.rx_buffer));
            dbg_uart_parser(dbg_uart.rx_buffer, dbg_uart.rx_ptr_in);
        ...
        if (fb_uart.rx_ready_tmr == 1) {
            ...
            fb_uart_parser(fb_uart.rx_buffer, fb_uart.rx_ptr_in);
            fb_uart.rx_ptr_in = 0;
        }
        glb_tmr_1sec++;
    }
...
void RunInference(float *data, size_t data_size)
{
    float output[10];
    ...
    int ret = aiRun(data, output);

    uint32_t cycles = dwtGetCycles();
    restoreInts(ints);
    
    if (ret !=0 ) {
        TRACE(("[RunInference] error: %d\n", ret));
        return;
    }
    ...
    auto out_vect = fbb.CreateVector((float*) output, 10);
...
void fb_uart_parser(uint8_t *buffer, size_t bufferlen)
{
    ...
    if (req->cmd() == MnistProt::Command_CMD_GET_STATS) {
        ...
        RunInference(p, digit->digit()->size());
    }
}
